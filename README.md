...
- 实例分割：逐点单击绘制多边形，单击起点、双击或 `Enter` 闭合，右键取消当前绘制，`Backspace` 撤销顶点；将鼠标移到已有多边形边线后按 `I` 可插入新顶点，随后可拖动该顶点调整轮廓。插点按键可在“快捷键”菜单中修改或禁用，`Ctrl` 仅用于临时切换为手形平移。
...
类别名称、颜色、快捷键和工作台设置由 `config.yaml` 配置。运行期间产生的临时标签默认位于 `utils/temp_folder`，退出前请通过界面的保存功能导出正式标签。
