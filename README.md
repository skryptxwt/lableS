...
- 实例分割：逐点单击绘制多边形，单击起点、双击或 `Enter` 闭合，右键取消当前绘制，`Backspace` 撤销顶点；选中已有多边形后，`Shift+单击` 任意边线可插入新顶点，随后可拖动该顶点调整轮廓。`Ctrl` 仅用于临时切换为手形平移。
...
四种任务均可双击已有标注打开类别选择；实例分割在绘制草稿期间双击仍用于闭合当前轮廓。
